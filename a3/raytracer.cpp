...
Primitive* Engine::Raytrace( Ray& a_Ray, Color& a_Acc, int a_Depth, float a_RIndex, float& a_Dist )
{
	...
	if (prim->IsLight())
	{
		...
		a_Acc = getHaze(a_Dist) * Color( 1, 1, 1 ) + (1 - getHaze(a_Dist)) * getHazeColor();
	}
	else
	{
		...
		for ( int l = 0; l < m_Scene->GetNrPrimitives(); l++ )
		{
			...
			if (p->IsLight()) 
			{
				...
				if (shade > 0)
				{
					...
					{
						if (prim->GetType() == Primitive::CUBE) {
							int tt = 0;
						}
						float dot = DOT( L, N );
						if (dot > 0)
						{
							...
							a_Acc += getHaze(a_Dist) * diff * prim->GetMaterial()->GetColor() * light->GetMaterial()->GetColor() + (1 - getHaze(a_Dist)) * getHazeColor();
						}
					}
					...
					if (prim->GetMaterial()->GetSpecular() > 0)
					{
						...
						if (dot > 0)
						{
							...
							a_Acc += getHaze(a_Dist) * spec * light->GetMaterial()->GetColor() + (1 - getHaze(a_Dist)) * getHazeColor();
						}
					}
				}
			}
		...
		if ((refl > 0.0f) && (a_Depth < TRACEDEPTH))
		{
			...
			a_Acc += getHaze(a_Dist) * refl * rcol * prim->GetMaterial()->GetColor() + (1 - getHaze(a_Dist)) * getHazeColor();
		}
		...
		if ((refr > 0) && (a_Depth < TRACEDEPTH))
		{
			...
			if (cosT2 > 0.0f)
			{
				...
				a_Acc += getHaze(a_Dist) * rcol * transparency + (1 - getHaze(a_Dist)) * getHazeColor();
			}
		}
	}
