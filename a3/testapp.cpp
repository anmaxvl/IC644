...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow)
{
	...
	wc.lpszClassName = (LPCWSTR)"raytracer";
	if (!RegisterClass(&wc)) return FALSE;
	rect.left = rect.top = 0;
	rect.right = SCRWIDTH, rect.bottom = SCRHEIGHT;
	AdjustWindowRect( &rect, WS_POPUP|WS_SYSMENU|WS_CAPTION, 0 );
	rect.right -= rect.left, rect.bottom -= rect.top;
	wnd = CreateWindowEx( 0, (LPCWSTR)"raytracer", (LPCWSTR)"raytracer", WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX & ~WS_THICKFRAME,
	...
	tracer->setHazeCoefficient(0.02);
	tracer->setHazeColor( Raytracer::Color(0.0, 0.0, 0.5) );
	int tpos = 60;
	// go
	while (1)
	{
